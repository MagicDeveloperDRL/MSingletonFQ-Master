...
namespace MSingletonFQ
{
    ...
    public class PETimer
    {
        ...
        private static readonly string lockTid = "lockTid";//线程锁
        private DateTime startDateTime = new DateTime(1970, 1, 1, 0, 0, 0, 0);
        private double nowTime;
        private Timer srvTimer;
        private int tid;//全局的任务ID（int类型存在最大类型）
        private List<int> tidLst = new List<int>();
        private List<int> recTidLst = new List<int>();//待回收任务ID列表

        private static readonly string lockTime = "lockTime";//线程锁
        //定时任务的临时缓存列表
        ...
        /// <summary>
        /// 检查是否临时缓存中是否存在定时任务，若存在则检查是否应当执行
        /// </summary>
        ...
        /// 添加定时任务
        /// </summary>
        /// <param name="callback">定时完成后执行的操作</param>
        ...
        class PETimeTask
        {
            public int tid;//任务ID，用来删除、暂停指定定时任务等
            public Action<int> callback;//回调函数
            public double destTime;//定时任务应该结束的时刻，单位：毫秒
            public double delay;//延迟的时间
            public int count;//执行的次数
            /// <summary>
            /// 构造函数
            /// </summary>
